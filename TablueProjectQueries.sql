...
SELECT
    ...
    MAX(CAST(total_cases AS BIGINT) * 100.0 / CAST(Population AS BIGINT)) AS PercentPopulationInfected
FROM project. .CovidDeaths
...
SELECT
    location,
    MAX(CAST(total_cases AS bigint)) AS HighestInfectionCount,
    MAX(CAST(total_cases AS bigint)) * 100.0 / MAX(CAST(population AS bigint)) AS PercentPopulationInfected,
